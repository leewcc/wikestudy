...
@WebServlet("/dist/jsp/common/user/user_topic/my_answers_get")
public class GetMyAnswer extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		//第四步：	获取我的回答
			PageElem<Comment> myAnswers = cs.getMyAnswers(uid, ut, cp);
			List<Label> labels = ls.getList();
			
		//第五步：	将我的回答set进请求里，转发到我的回答页面
			request.setAttribute("labels", labels);
			request.setAttribute("myAnswers", myAnswers);
			request.getRequestDispatcher("my_answers.jsp").forward(request, response);
