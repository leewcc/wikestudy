 ...
 */
@WebServlet("/dist/jsp/teacher/course/course_update")
public class UpdateCourse extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			if (couName == null || couName.trim().equals("") || couName.length() >= 40) {
				flag = false;
			}
			...
			cou.setCouPricUrl(couPricUrl.substring(couPricUrl.lastIndexOf("/")));
			...
			for (int i = 0; i < chaListJ.length(); i++) {
				...
				if (chaName == null || chaName.trim().equals("") || chaName.length() >= 50) {
					flag = false;
				}
			...
			for (int i = 0; i < secListJ.length(); i++) {
				...
				if (secName == null || secName.trim().equals("") || secName.length() >= 50) {
					flag = false;
				}
			...
			if (!flag) {
				...
				response.getWriter().println("请填写课程名和章节信息(不超过40字)");
				return ;
			}
